 ...
 */    
#define UART_DEBUG_ENABLE    (true)//(false)
 ...
 */    
    ...
    #define DebugPrintfInit()  { Cy_SCB_UART_Init(DEBUG_UART_HW, &DEBUG_UART_config, &DEBUG_UART_context); Cy_SCB_UART_Enable(DEBUG_UART_HW);}
	#define KitUartInit()  { Cy_SCB_UART_Init(KIT_UART_HW, &KIT_UART_config, &KIT_UART_context); Cy_SCB_UART_Enable(KIT_UART_HW);}
	/* Function macro that sends formatted output to STDOUT */
    #define DebugPrintf(...)     (printf(__VA_ARGS__))
    
    
    #define WAIT_FOR_UART_TX_COMPLETE    while(Cy_SCB_GetNumInTxFifo(DEBUG_UART_HW) + \
                                    Cy_SCB_GetTxSrValid(DEBUG_UART_HW))
#if CY_CPU_CORTEX_M4
        ...
        #define Task_Debug_Tickless_Idle_Readiness()       \
                     ((Cy_SCB_UART_GetNumInTxFifo(DEBUG_UART_HW) == 0u) && \
                     (Cy_SCB_GetTxSrValid(DEBUG_UART_HW)== 0u))
